...
def main():
    ...
    if st.session_state.cv_data:
        ...
        # === SÉLECTION GLOBALE DES DOMAINES ET SECTEURS (apparaissent sur TOUS les slides) ===
        ...
        with col_dom:
            ...
            selected_domaines = []
            st.markdown("**Domaines d'expérience**")
            for dom in domaine_list:
                if st.checkbox(dom, key=f"dom_{dom}"):
                    selected_domaines.append(dom)

        with col_sec:
            secteur_list = [l.strip() for l in d["SECTEURS_EXPERIENCE"].split('\n') if l.strip()]
            selected_secteurs = []
            st.markdown("**Secteurs d'expérience**")
            for sec in secteur_list:
                if st.checkbox(sec, key=f"sec_{sec}"):
                    selected_secteurs.append(sec)
        ...
        # Préparation des listes d'éléments
        ...
        # Initialisation des éléments sélectionnés par slide
        slide_elements = []
        for slide_num in range(1, 4):
            st.markdown(f"### Slide {slide_num}")

            st.markdown("**Expériences Pertinentes**")
            selected_experiences = display_element_checkboxes(f"Expériences pour Slide {slide_num}", exp_list, f"exp_slide_{slide_num}")

            st.markdown("**Références Pertinentes**")
            selected_references = display_element_checkboxes(f"Références pour Slide {slide_num}", ref_list, f"ref_slide_{slide_num}")

            st.markdown("**Diplômes**")
            selected_diplomes = display_element_checkboxes(f"Diplômes pour Slide {slide_num}", dip_list, f"dip_slide_{slide_num}")
